...
def evaluate_policy_discounted(env, policy, discount_factor, trials = 1000):
    ...
    for _ in range(trials):
        ...
        while not done:
            observation, reward, done, info = env.step(policy[observation])
            total_reward += (discount_factor ** k) * reward
            k += 1
    return total_reward / trials
...
def VI_worker_v3(VI_server, data, start_state, end_state):
        ...
        for state in range(start_state, end_state + 1):      
            ...
            for action in range(A):
                reward_current = env.GetReward(state, action)
                successors = env.GetSuccessors(state, action)
                sum_trans = 0
                for i in successors:
                    sum_trans += i[1] * V[i[0]]
                v_t = reward_current + beta * sum_trans
                if v_t > max_v:
                    max_v = v_t
                    max_a = action
        
            VI_server.update.remote(state, max_v, max_a)
...
def fast_value_iteration(env, beta = 0.999, epsilon = 0.01, workers_num = 4):
    ...
    workers_list = [VI_worker_v3.remote(VI_server, data_id, start_states[i], start_states[i] + batch_size) for i in range(workers_num)]
                               
    error = float('inf')
    while error > epsilon:
        finished_worker_ids= ray.wait(workers_list, num_returns = workers_num,timeout = None)[0][0]
        finish_workers = ray.get(finished_worker_ids)
